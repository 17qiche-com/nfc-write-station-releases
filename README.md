...
打开 DMG，将“NFC 写卡台”拖入“应用程序”。旧版用户应先退出应用，再替换同名应用；不要删除应用数据目录。

## 应用内更新

v0.10.3 及后续版本可在左下角或应用菜单中点击“应用更新”，无需 GitHub 账号或访问令牌。确认新版本后，应用会下载、校验签名、安装并重新启动。

写卡或任务检查进行中请先暂停，等待当前卡片操作结束。更新不清除任务数据和历史记录。v0.11.0 起卡片密码改为本机 AES-GCM 加密文件，不再访问 macOS 钥匙串；旧任务中的密码需要重新录入一次。

v0.10.2 及更早版本请从本仓库手动下载安装一次，之后使用新的公开更新源。
